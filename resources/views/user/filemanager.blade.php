	    ...
	    <div class="modal-header">
	    	<div class="row">
	    		...
	    		<div class="col-sm-6">
					...
					<a style="float: right;margin-bottom: 0;margin-right: 0;border-radius: 0;border-bottom: 0;" href="{{url('/user/filemanager?directory='.$parent)}}" data-toggle="tooltip"  id="button-refresh" class="btn btn-default"><i class="fa fa-refresh"></i></a>    	    	
		      	</div>
	    	</div>
	    </div>
      		...
      		@if( !empty($media_files['files'] ) )
	      		@for ($i = 0; $i < count($media_files['files']); $i++)
				    ...
				    @else 
				    	<div class="col-sm-2 col-xs-6 text-center">
				          	<div class="upload-img-items">
					          	<a href="javascript:void(0);" data-directory="/user/filemanager?directory={{$media_files['files'][$i]['name'] }}" class="directory">
...
<script type="text/javascript">
	...
	$('.custom-pagination a').on('click', function(e) {
		...
		var target_url = base_url+"/user/filemanager?page="+encodeURIComponent(page)+"&target_id="+encodeURIComponent(target_id)+"&directory="+encodeURIComponent(directory);
	...
	$('#button-upload').on('click', function() {
		...
		timer = setInterval(function() {
			if ($('#form-upload input[name=\'file_name\']').val() != '') {
				clearInterval(timer);
				$.ajax({
					url: "{{url('/user/filemanager/upload')}}",
					type: 'post',
					dataType: 'json',
	...
	$('#modal-image #button-delete').on('click', function(e) {
		...
		if (confirm('Are you sure ?')) {
			...
			$.ajax({
				url: "{{url('/user/filemanager/delete')}}",
				type: 'post',
				dataType: 'json',
	...
	$('#button-folder').on('shown.bs.popover', function() {
		$('#button-create').on('click', function() {
			...
			$.ajax({
				url: "{{url('/user/filemanager/folder/create')}}",
				type: 'post',
				dataType: 'json',
